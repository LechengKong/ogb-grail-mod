...
from utils.graph_utils import incidence_matrix, incidence_matrix_coo, remove_nodes, ssp_to_torch, serialize, deserialize, get_edge_count, diameter, radius
...
def sample_neg_wiki(adj_list, edges, num_neg_samples_per_link=1, max_size=1000000, constrained_neg_prob=0):
    ...
    while len(neg_edges) < num_neg_samples_per_link * len(pos_edges):
        ...
        v_heads = adj_list[rel].row
        v_tails = adj_list[rel].col

        if np.random.uniform() < constrained_neg_prob:
            if np.random.uniform() < 0.5:
                neg_head = np.random.choice(v_heads)
            else:
                neg_tail = np.random.choice(v_tails)
        else:
            if np.random.uniform() < 0.5:
                neg_head = np.random.choice(n)
            else:
        ...
        if neg_head != neg_tail and np.sum(v_tails[v_heads == neg_head] == neg_tail) == 0:
            neg_edges.append([neg_head, neg_tail, rel])
            pbar.update(1)
...
def sample_neg_one(adj_list, edge, n, constrained_neg_prob=0):
    neg_head, rel, neg_tail = edge[0], edge[1], edge[2]
...
def links2subgraphs_wiki(A, graphs, params, max_label_value=None):
    '''
    extract enclosing subgraphs, write map mode + named dbs
    '''
    max_n_label = {'value': np.array([0, 0])}
    ...
    A_inc = incidence_matrix_coo(A)
    A_inc += A_inc.T
    BYTES_PER_DATUM = get_average_subgraph_size_wiki(100, list(graphs.values())[0]['pos'], A_inc, params) * 2
    links_length = 0
    for split_name, split in graphs.items():
        links_length += (len(split['pos']) + len(split['neg'])) * 2
    map_size = links_length * BYTES_PER_DATUM
    ...
    def extraction_helper(A, links, g_labels, split_env):
        ...
        with mp.Pool(processes=None, initializer=intialize_worker, initargs=(A, params, max_label_value)) as p:
            args_ = zip(range(len(links)), links, g_labels)
            for (str_id, datum) in tqdm(p.imap(extract_save_subgraph_wiki, args_), total=len(links)):
                max_n_label['value'] = np.maximum(np.max(datum['n_labels'], axis=0), max_n_label['value'])
                subgraph_sizes.append(datum['subgraph_size'])
                enc_ratios.append(datum['enc_ratio'])
                num_pruned_nodes.append(datum['num_pruned_nodes'])
    ...
    for split_name, split in graphs.items():
        ...
        db_name_pos = split_name + '_pos'
        split_env = env.open_db(db_name_pos.encode())
        extraction_helper(A_inc, split['pos'], labels, split_env)

        logging.info(f"Extracting enclosing subgraphs for negative links in {split_name} set")
        labels = np.zeros(len(split['neg']))
        db_name_neg = split_name + '_neg'
        split_env = env.open_db(db_name_neg.encode())
        extraction_helper(A_inc, split['neg'], labels, split_env)
...
def get_average_subgraph_size_wiki(sample_size, links, A, params):
    total_size = 0
    for (n1, n2, r_label) in links[np.random.choice(len(links), sample_size)]:
        nodes, n_labels, subgraph_size, enc_ratio, num_pruned_nodes = subgraph_extraction_labeling_wiki((n1, n2), r_label, A, params.hop, params.enclosing_sub_graph, params.max_nodes_per_hop)
        datum = {'nodes': nodes, 'r_label': r_label, 'g_label': 0, 'n_labels': n_labels, 'subgraph_size': subgraph_size, 'enc_ratio': enc_ratio, 'num_pruned_nodes': num_pruned_nodes}
        total_size += len(serialize(datum))
    return total_size / sample_size


def extract_save_subgraph_wiki(args_):
    idx, (n1, n2, r_label), g_label = args_
    nodes, n_labels, subgraph_size, enc_ratio, num_pruned_nodes = subgraph_extraction_labeling_wiki((n1, n2), r_label, A_, params_.hop, params_.enclosing_sub_graph, params_.max_nodes_per_hop)
